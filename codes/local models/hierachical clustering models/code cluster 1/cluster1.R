#This is the r version for utilising rotation forest for building
#hierarchical clustering based models for testing on the cluser 1
#In Africa data
...
rotationForest <- function(data,k=4,numTrees=35,bootstrapRate=0.75,type="class"){
...
dataTraining <- read.csv("withlabels.csv")
#Select cluster 1 in Africa as testing data
dataTesting <- dataTraining[(dataTraining$CONT == 'AF') & (dataTraining$clustervalue == 1),]
#Select a group of clusters that are not in Africa as training data
clusterNumber <- c(1,2,3)
total_Train <- data.frame()
RMSE_ <- vector(mode="double", length=0)
for (i in 1: length(clusterNumber)){
  print(i)
  dataTrain <- dataTraining[(dataTraining$CONT != 'AF') & (dataTraining$clustervalue == clusterNumber[i]),]
  total_Train <- rbind(total_Train, dataTrain)
  
  trn.data <- total_Train[,c("mean_HYP","mean_LOP","mean_FCT_HOD" ,"mean_FCT_AL", "mean_FCT_OL","mean_FCT_SF","mean_FCT_OT","mean_FCT_CM","NPP")]
  test.data <- dataTesting[,c("mean_HYP","mean_LOP","mean_FCT_HOD" ,"mean_FCT_AL", "mean_FCT_OL","mean_FCT_SF","mean_FCT_OT","mean_FCT_CM","NPP")]
  model <- rotationForest(trn.data,k=4,type="anova")
  predictiony <- rf.predict(model,test.data,type="regression")
  RMSE_ <- c(RMSE_, regression.perf_eval(dataTesting$NPP,predictiony)[2])
}

dataTesting$Predict_RoF <- predictiony
regression.perf_eval(dataTesting$NPP,predictiony)
